...
async def create_user(db:db_dependancy,
    ...
    create_user_model=Buyers(firstname=create_user_request.first_name,
...
@router.get('/allbuyers')
async def get_all_buyers(db:db_dependancy,current_user:Annotated[dict,Depends(get_current_user)]):
    user=db.query(Buyers)
    return user.all()